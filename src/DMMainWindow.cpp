...
void DMMainWindow::toggle_subscription() {
  if (this->subscriber == nullptr) {
    ...
    for (auto w = this->dm_canvases.begin(); w != this->dm_canvases.end(); ++w) {
      connect(this->subscriber, &Subscriber::new_danmaku, qobject_cast<DMCanvas*>(*w), &DMCanvas::new_danmaku);
    }
    connect(this->subscriber, &Subscriber::started, this, &DMMainWindow::on_subscription_started);
    connect(this->subscriber, &Subscriber::finished, this, &DMMainWindow::on_subscription_stopped);
    connect(this->subscriber, &Subscriber::new_alert, this, &DMMainWindow::on_new_alert);
...
void DMMainWindow::init_canvases() {
  ...
  for (int i = 0; i < screens.size(); i++) {
    if (this->screenBoxes[i]->checkState()) {
      ...
      if (this->subscriber != nullptr) {
        connect(this->subscriber, &Subscriber::new_danmaku, canvas, &DMCanvas::new_danmaku);
      }
      this->app->connect(screen, &QScreen::availableGeometryChanged, this, &DMMainWindow::reset_canvases);
...
DMTrayIcon::DMTrayIcon(QWidget* parent) : QSystemTrayIcon(parent) {
  ...
  connect(this, &DMTrayIcon::activated, this, &DMTrayIcon::on_activated);
  this->show();
}
